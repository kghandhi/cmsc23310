...
class Node:
  ...
  def start(self):
    ...
    #for friend in self.peer_names:
    #self.req.send_json({"type": "log", 'debug': {'event': 'getting', 'node': self.name, 'key': peer_names, 'value': 14}})
  ...
  def handle(self, msg_frames):
    ...
    #self.req.send_json({'type': 'log', 'debug': {'event': 'getting', 'node': self.name, 'key': [1,2], 'value': 2}})
