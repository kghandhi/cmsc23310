    ...
    def deliver_message(self, N, msg):
        if msg.typ == "PREPARE":
            if msg.n > self.n_int:
                ...
                # else:
                #     high_p = []
                new_msg = Message(msg.pi_v, "PROMISE", self.ID, msg.src, msg.n, self.accs)
            else:
                new_msg = Message(msg.pi_v, "REJECTED", self.ID, msg.src, msg.n, [])
            queue_message(N, new_msg)

        if msg.typ == "ACCEPT":
            if n_int < msg.n:
                new_msg = Message(msg.pi_v, "ACCEPTED", self.ID, msg.src, msg.n, [])
                self.accs.append(msg.v, msg.n)
            else:
                new_msg = Message(msg.pi_v, "REJECTED", self.ID, msg.src, msg.n, [])
            queue_message(N, new_msg)
        else:
